...
public class FamilyTreeBuilder implements TreeBuilder {
	 ...
	 * @throws Exception
	 */
	private void addPerson(Person p, Relationship relation) {
		if(p == null)
			throw new CustomException("Person not yet created...");
	 ...
	 */
	@Override
	public Relationship createRelationship(RelationshipType rt, String parentId, String childId, String familyId) {
		...
		if(familyPeople.get(parent.getId()) == null && familyPeople.get(child.getId()) == null)
			return null;
		Relationship rs = new Relationship(rt);
	...
	public String getFamilyPerson(String familyId) {
		try {
			...
			throw new CustomException("Family ID is invalid.");
		}
	}
	...
	public String getFamilyRoot(String familyId) {
		...
		if(isFamily(familyId))
			root = families.get(familyId).getRoot();
		return root;
	}
	 ...
	 * @throws Exception
	 */
	public static boolean check(String parentId, String childId) {
		...
		try {
			...
			throw new CustomException("Person doesn't exist.");
		}
	}
